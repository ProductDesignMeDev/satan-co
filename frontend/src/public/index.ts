export { default as Logo } from "./Logo.png";
export { default as Banner } from "./Banner.jpeg";
export { default as FuegoFooter } from "./FuegoFooter.png";
export { default as NubeFooterDerecha } from "./NubeFooterDerecha.png";
export { default as NubeFooterIzquierda } from "./NubeFooterIzquierda.png";
export { default as NubesMobileMenu } from "./NubesMobileMenu.png";
export { default as IconInstagram } from "./IconInstagram.png";
export { default as Plantita } from "./Plantita.jpg";
export { default as Error404 } from "./Error404.svg";
export { default as FlechaIzquierda } from "./FlechaIzquierda.svg";
export { default as IconCannabis } from "./IconCannabis.svg";
export { default as HumoFooterIzquierda } from "./HumoFooterIzquierda.svg";
export { default as HumoFooterDerecha } from "./HumoFooterDerecha.svg";
export { default as HumoEdadIzquierda } from "./HumoEdadIzquierda.svg";
export { default as HumoEdadDerecha } from "./HumoEdadDerecha.svg";
export { default as Ellipse } from "./Ellipse.svg";
export { default as EllipseDerecha } from "./EllipseDerecha.svg";
export { default as Premio } from "./Premio.png";
export { default as CopaMar } from "./CopaMar.png";
export { default as CopaPocoHumo } from "./CopaPocoHumo.png";
export { default as FlechaIzquierd } from "./FlechaIzquierda.png";
export { default as FlechaDerecha } from "./FlechaDerecha.png";
